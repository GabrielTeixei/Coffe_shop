        ...
        <div className='absolute top-0 left-0 w-full h-screen bg-black/50 text-white'>
          <nav className='nav_style text-white lg:flex items-center p-5 justify-between'>
            <div className='flex items-center justify-between'>
              <div className='flex border-1 gap-2 p-1 items-center bg-clip-padding backdrop-filter backdrop-blur-xl bg-opacity-60 rounded-md relative'>
                <CiSearch size={35} />
                <input type='text' placeholder='Search' className='outline-none bg-transparent' value={searchInput} onChange={handleSearchChange}
                />
                {suggestions.length > 0 && (
                  <ul className='absolute top-[3rem] left-0 w-full bg-white text-black rounded-md shadow-lg z-10'>
                    {suggestions.map((suggestion, index) => (
                      <li key={index} className='p-2 hover:bg-gray-200 cursor-pointer' onClick={() => handleSuggestionClick(suggestion)}> {suggestion}
                      </li>
                    ))}
                  </ul>
                )}
